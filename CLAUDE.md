...
Phase 3の方針（**旧マインドマップエンジンはロジックをSolid化せず、そのままlib配下にvanilla JSとして残し、Solidからは起動関数を呼ぶだけ**）で進めて問題ないですか？　ここを「Solidのリアクティブな状態管理に全面書き換え」にするなら、フェーズ構成が大きく変わります（かなりの工数になります）。

この前提でよければ、**Phase 1から順番に**、1フェーズずつ実際のファイル移動・編集に着手します。まずPhase 1（vite.config.js修正）から始めてよいですか？
