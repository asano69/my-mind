...
//
// createEngineCommands() closes this module's previously module-level
// state (the repo Map's command closures, and Pan's mutable
// codes/interval/panKeyboardScope) into a per-instance factory, per
// docs/mind-map-core-engine-library/01-plan.md's Step 5 -- same pattern
// as newMouse.js's createMouseController()/newClipboard.js's
// createClipboardController(). The default singleton instance at the
...
// wrapped in the canonical (primary) tag. Stateless, so it stays a
// plain module-level helper rather than moving inside the factory.
const STYLE_TAGS = {
  bold: ["b", "strong"],
  italic: ["i", "em"],
  underline: ["u"],
...
export function createEngineCommands({
  ...
  // small closure-local object rather than a repo entry with its own
  // mutable state, since Pan needs to track "which of WASD are
  // currently held" independently of the generic execute(e) shape
  // every other command here uses.
  let panKeyboardScope = globalThis.window ?? null;

  // Called by the keyboard controller's init()/dispose() so the keyup
  // listener that ends a pan is scoped to the same container keydown
  // is scoped to, mirroring command/command.js's setKeyboardScope().
  function setPanKeyboardScope(scope) {
  ...
  // Called by the keyboard controller's dispose() so an in-progress pan
  // doesn't keep its setInterval running past unmount.
  function disposePan() {
    pan.dispose();
  }
  ...
  // but no combined getter of its own. Bound to this instance's own
