// newKeyboard.js — keyboard shortcuts for the mindmap engine.
//
// createKeyboardController() closes this module's previously
// module-level state (the commands array's closures, extraCommandRepo,
// and the focus self-heal guard's pendingRestore handle) into a
// per-instance factory, per docs/mind-map-core-engine-library/01-plan.md's
// Step 5 -- same pattern as newMouse.js's createMouseController()/
// newClipboard.js's createClipboardController(). The default singleton
// instance at the bottom of this file preserves every existing
// `import * as newKeyboard from "./newKeyboard.js"` call site unchanged
...
export function createKeyboardController({
  ...
  // as a fallback, same as sharedCommandRepo above.
  let extraCommandRepo = null;
  function registerExtraCommands(repo) {
  ...
  // engineCommands.js's runStyleCommand()) in one place instead of
  // duplicating it here.
  commands.push(
    {
      mode: "editing",
  ...
  function handleEvent(e) {
    ...
    // engineCommands.js), extended with `keys` for hotkey use. Only
    // consulted while not editing -- none of these are meant to fire
