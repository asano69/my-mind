...
- 全木伝播は葉ノード編集の約26倍だが、絶対値としては39msで収まっている。60fpsのフレーム予算（約16.7ms）は超えるため、実ブラウザでは色変更操作の瞬間にフレームドロップが1〜2回起きる可能性はあるが、体感で「固まる」というレベルではない。
- **この数値は実際のブラウザコストを過小評価している点に注意。** このテストの`node()`スタブは`offsetWidth`/`scrollWidth`などを固定値で即座に返すプレーンオブジェクトであり、実DOMの`getBBox()`/`offsetWidth`読み取りが引き起こすレイアウト（reflow）コストを一切含んでいない。実ブラウザでは781ノード全体のreflowが発生するため、実測はこれより大きくなる可能性が高い。このベンチマークは「ロジック上のオーバーヘッド」の下限を示すものであり、実ブラウザでの体感を保証するものではない。
- 通常のマップサイズ（数十〜200ノード程度を想定、README「大量ノードの場合パフォーマンスに問題がでる」の対象規模）では、この程度の伝播コストはさらに小さくなる。

### Phase 0 の結論とPhase 1着手可否

Phase 0の計測でわかったのは「現状の実装はパフォーマンス上壊れていない」ということだけである。葉ノード編集・collapse切り替えは既に良好に局所化されており（訪問数）、継承プロパティ変更（全木再計算）も実測ではミリ秒オーダー（781ノードで39ms）に収まっている。**したがって、パフォーマンス問題はPhase 1〜6着手の理由にもならないし、着手を止める理由にもならない。**

doc08本文が挙げている本来の動機は性能ではなくDX（開発体験・保守性）である: `createMemo`内のDOM副作用、`_sideVersion`/`_contentVersion`/`_childrenVersion`/`dirtyVersion`/`layoutVersion`のようなプレーンなバージョンカウンタの乱立とbump漏れによる再発しやすいバグ、rAF二重待ちハックの散在、`registerXxx`/`dispose`ブリッジパターンの拡散——これらはPhase 0のベンチマークでは測定できない、コードベースの認知負荷・保守性に関する判断であり、CLAUDE.mdのWork in progress（「pubsub時代の非リアクティブなグローバル状態の発見とリファクタリング」）が指す課題そのものである。

この観点に立てば、Phase 0が果たすべき役割は「Phase 1に進むかどうかの意思決定」ではなく、「Phase 1に進んでも性能面の不意打ちがないことを保証する安全網」である。その役割は今回の計測で満たされた。Phase 1着手はDX上の判断としてこのまま進めてよい。継承プロパティ変更のcolor/textColor=121（訪問数）・39ms（781ノード実測）は、Phase 3以降で新実装が性能面で劣化していないかを確認する際の基準値として引き続き使う。
